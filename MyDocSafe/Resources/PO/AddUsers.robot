...
Add new
    ...
    wait until element is enabled    ${Addnewbutton}     45s    #Wait for add new button
    sleep    5s
    double click element    //span[text() = 'Add new']    #Double click Add new button

Input email

    wait until element is enabled    ${Contacttextbox}  45s    #Wait for Email text field
...
Click Create the client portal and invite client users button
    ...
    wait until element is enabled    ${Createtheclientportalandinviteclientusersbutton}  45s    #Wait for create_client_portal_invite_client_users button
    click element    ${Createtheclientportalandinviteclientusersbutton}    #click create_client_portal_invite_client_users button
    wait until element is enabled    //span[text() = 'Looks like this folder is empty.']    45s    #Wait for the next page to load
