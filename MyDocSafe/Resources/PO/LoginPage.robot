...
Enter Email    [Arguments]    ${Email}
    ...
    wait until element is enabled    ${EmailTextBox}    45s
    input text    //input[@name = 'email']    ${Email}    #Email address text box

Enter Password    [Arguments]    ${Password}

    wait until element is enabled    ${PasswordTextBox}  45s
    input text    ${PasswordTextBox}    ${Password}    #Password text box

Click Signin Button

    wait until element is enabled    ${SigninButton}  45s    #Wait for Signin Button to enable
    sleep    1s
    click element    ${SigninButton}    #Signin Button

Home screen

   wait until element is visible    //button[@aria-label = 'Identity verification']    45s    #Wait for Home page to load
