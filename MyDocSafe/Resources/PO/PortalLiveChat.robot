...
Select Portal    [Arguments]    ${Portal_Name}
    ...
    wait until element is enabled    //button[contains(@aria-label, '${Portal_Name}')]  45s
    click element    //button[contains(@aria-label, '${Portal_Name}')]

Select Portal User   [Arguments]    ${Portal_User}

    wait until element is enabled    //a[contains(@aria-label, '${Portal_User}')]  45s
    click element    //a[contains(@aria-label, '${Portal_User}')]

Click Chat Button

    wait until element is enabled    (//span[text() = 'Chat']//parent::translate//parent::a)[2]    45s
    sleep    2s
    click element    (//span[text() = 'Chat']//parent::translate//parent::a)[2]

Click Start Chat button

    run keyword and ignore error    wait until element is enabled    //span[text() = 'Start chat']//parent::translate//parent::button  45s
    sleep    2s
...
Send Message
    ...
    Scroll Down
    wait until element is enabled    ${ChatInputBox}  45s
    ${Message}    randomword
    input text    ${ChatInputBox}    ${Message}
    press keys    ${ChatInputBox}    ENTER
    wait until element is enabled    //span[@class = 'mdsChatMessage ng-binding'][contains(text(), '${Message}')]  45s
    [Return]    ${Message}
...
Click Accept Invite button
    ...
    wait until element is enabled    ${AcceptInviteButton}    45s
    sleep    1s
...
Select a contact    [Arguments]    ${Recipient}
    ...
    wait until element is enabled    ${SelectContactTextField}    45s
...
Click Accept Invitation Button
    ...
    wait until element is enabled    ${AcceptInvitationButton}  45s
...
Add Randaom Recipient details
    ...
    Wait Until Element Is Enabled    ${Last_Name_1}  45s
