...
Wait for document to load
    ...
    wait until element is not visible    ${Documentloading}    45s

Click Send document button

    wait until element is enabled  ${SendDocumentButton}  45s
    click element    ${SendDocumentButton}
    sleep    2s

Verify Document is sent

    wait until element is enabled    ${DocumentSentMessage}  45s
    capture page screenshot    Document_Sent.png

Click Return to your documents button

    wait until element is enabled    ${ReturnToYourDocumentsButton}  45s
    click element    ${ReturnToYourDocumentsButton}

Click Next Button from Send for Signing

    wait until element is enabled    ${NextSendForSigningButton}  45s
