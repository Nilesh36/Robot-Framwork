...
Enter text in the search box    [Arguments]    ${FolderName}
    ...
    wait until element is enabled  ${Searchtexbox}    45s    #Wait for Search field
...
Verify the folder is created
    ...
    press keys    None    TAB
    wait until element is enabled    ${Folder}  45s
    capture page screenshot    Folder_creation.png

Select the folder

    wait until element is enabled    ${Folder}  45s    #wait for the folder name to appear
