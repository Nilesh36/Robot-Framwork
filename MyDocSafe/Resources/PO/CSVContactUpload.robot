...
Upload CSV File
    ...
    wait until element is enabled    //input[@type = 'file']  45s
    choose file    //input[@type = 'file']    ${CURDIR}${/}2_contacts.csv

Import contacts    [Arguments]    ${Index}    ${Value}

    wait until element is enabled    (//select[@ng-model = 'modalScope.mappingData[key]'])[${Index}]  45s
    select from list by value    (//select[@ng-model = 'modalScope.mappingData[key]'])[${Index}]    ${Value}

Click Continue Button

    wait until element is enabled    ${ContinueButton}  45s
    click element    ${ContinueButton}
    Sleep    1s

Verify the Import completion message

    wait until element is enabled    ${ImportCompletemsg}  45s
    capture page screenshot    Import_Complete.png

Click Close

    wait until element is enabled    ${CloseButtonContacts}  45s
