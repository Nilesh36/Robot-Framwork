...
File Upload    [Arguments]    ${FILENAME}
    ...
    wait until element is enabled    ${Uploadanewfilebutton}    45s    #Wait for Upload new file button
...
Click Close Button
    ...
    sleep    2s
    wait until element is enabled    ${CloseButton}  45s
    click element    ${CloseButton}

Upload More file    [Arguments]    ${FILENAME}

    wait until element is enabled    ${UploadMoreFileButton}  45s
