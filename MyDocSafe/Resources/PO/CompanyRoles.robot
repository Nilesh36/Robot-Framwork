...
Click Plus button from company roles
    ...
    wait until element is enabled    ${PlusButtonCompanyRoles}  45s
    sleep    2s
    click element    ${PlusButtonCompanyRoles}

Enter Role Name    [Arguments]    ${Rolename}

    wait until element is enabled    ${rolenametextbox}  45s
    input text    ${rolenametextbox}    ${Rolename}
    [Return]    ${Rolename}

Validate the Duplicate Role error message

    wait until element is enabled    ${DuplicateRoleError}  45s
    capture page screenshot    Duplicate_role.png

Click Submit Role

    wait until element is enabled    ${SubmitRoleButton}  45s
...
Close Dialog
    ...
    wait until element is enabled    ${CloseRoleDialog}  45s
    click element    ${CloseRoleDialog}

Delete Role    [Arguments]    ${RoleName}

     wait until element is enabled    //td[text() = '${RoleName}']//parent::tr/td[3]/div/button    45s
     click element    //td[text() = '${RoleName}']//parent::tr/td[3]/div/button
     wait until element is enabled    ${DeleteRole}  45s
     sleep    2s
     ${Count}    get element count    ${DeleteRole}
     click element    (//div[text() = 'Delete']//parent::div)[${Count}]
     wait until element is enabled    ${YesDeleteRole}  45s
     sleep    2s
...
Click Permission button
     ...
     wait until element is enabled    ${Permissionsbutton}  45s
     sleep    2s
...
Click Role options    [Arguments]    ${RoleName}
     ...
     wait until element is enabled    //td[contains(text(), '${RoleName}')]//parent::tr/td[3]/div/button    45s
     click element    //td[contains(text(), '${RoleName}')]//parent::tr/td[3]/div/button

Click Permission dropdown

     wait until element is enabled    ${PermissionsDropdown}  45s
     sleep  1s
     click element    ${PermissionsDropdown}

Select permissions    [Arguments]    ${Permission}    ${Index}

    wait until element is enabled    (//md-option/div[contains(text(), '${Permission}')])[${Index}]  45s
    sleep    1s
    click element    (//md-option/div[contains(text(), '${Permission}')])[${Index}]

Verify the permission is selected    [Arguments]    ${Permission}

     wait until element is enabled    //md-select-value[@class = 'md-select-value']/span/div[contains(text(),'${Permission}')]  45s
     sleep    1s
     capture page screenshot    Permissions.png

Click Save to Save the permission

    wait until element is enabled    ${SaveRoleButton}  45s
