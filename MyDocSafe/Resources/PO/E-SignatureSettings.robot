...
Check checkbox    [Arguments]    ${CheckboxName}
    ...
    wait until element is enabled    //div[@aria-label = '${CheckboxName}']/div[1]  45s
...
Update Expiration Period    [Arguments]    ${Days}
    ...
    wait until element is enabled     ${ExpirationPeriod}  45s
...
Click Update Esignature button
    ...
    wait until element is enabled    ${UpdateEsignatureSettingsbutton}  45s
    sleep  1s
...
Checkbox is checked    [Arguments]    ${CheckBoxName}
    ...
    wait until element is enabled    //md-checkbox[@aria-label = '${CheckboxName}'][@aria-checked = 'true']  45s

Checkbox is unchecked    [Arguments]    ${CheckBoxName}

    wait until element is enabled    //div[@aria-label = '${CheckboxName}'][@aria-checked = 'false']/div[1]  45s

Select 'Save as defaulting options'

    wait until element is enabled    //md-checkbox[contains(@aria-label, 'Save as default signing options.')]  45s
...
Click Add me button
    ...
    wait until element is enabled    ${AddmeButton}  45s
