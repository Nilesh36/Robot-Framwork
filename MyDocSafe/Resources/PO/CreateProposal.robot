...
Click Proposals plus button
    ...
    wait until element is enabled    ${ProposalsPlusButton}  45s
    click element    ${ProposalsPlusButton}

Enter Proposal Name

    ${Name}    randomword
    wait until element is enabled    ${ProposalNameTextBox}  45s
    input text    ${ProposalNameTextBox}    ${Name}

Enter Client Email

    wait until element is enabled    ${ProposalEmailTextBox}  45s
...
Enter Client First Name
    ...
    ${Name}    randomword
    wait until element is enabled    ${ClientFirstNameTextField}  45s
...
Click Next Button from Docs
    ...
    wait until element is enabled    ${NextButtonDocs}  45s
    sleep    1s
    click element    ${NextButtonDocs}

Choose Documents to Sign

    wait until element is enabled    ${TermsOfBusinessCheckBox}    45s
    sleep    1s
...
Click Skip Payment button
    ...
    wait until element is enabled    ${SkipPaymentButton}  45s
    sleep    1s
    click element    ${SkipPaymentButton}

Click Skip Fact form button

    wait until element is enabled    ${SkipFactForm}  45s
    sleep    1s
    click element    ${SkipFactForm}

Verify Proposal is created

    wait until element is enabled    {SentStatus}    45s
    capture page screenshot    Proposal_Created.png

Click Create Proposal Button

    wait until element is enabled    ${CreateProposalButton}  45s
    sleep    1s
...
Delete Proposal    [Arguments]    ${Email}
    ...
    wait until element is enabled    //p[contains(text(), '${Email}')]//parent::div//parent::md-list-item/div[5]  45s
    sleep    1s
    click element    //p[contains(text(), '${Email}')]//parent::div//parent::md-list-item/div[5]
    wait until element is enabled    ${DeleteProposalButton}  45s
    sleep    1s
    click element    ${DeleteProposalButton}
    sleep  1s
    wait until element is enabled    ${YesDeleteTheFolderPopup}  45s
