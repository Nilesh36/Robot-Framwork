...
Enter folder name
    ...
    sleep    2s
    wait until element is enabled    ${Foldernametextbox}  45s    #Wait for folder name field
    input text    ${Foldernametextbox}     ${FolderName}  #Enter folder name


Click Create a new folder button

    wait until element is enabled    ${CreateaNewfolderbutton}  45s    #Wait for Create a new folder button
