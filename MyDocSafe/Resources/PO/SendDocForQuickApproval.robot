...
Adding signing mail    [Arguments]    ${mail}
    ...
    wait until element is enabled    ${AddSigningbyEmailTextBox}  45s
...
Uploding large Document
    ...
    wait until element is enabled    ${AddNew}  45s
    Click Element    ${AddNew}
    wait until element is enabled    ${Document_Upload}  45s
    Run Keyword Until Success    Click Element    ${Document_Upload}
    wait_until_element_is_enabled    ${Upload_From_Computer}
    Choose File  ${Input_Document}  ${CURDIR}/Big Word 15MB.pdf
    sleep    2s
    wait until element is enabled    ${UploadMFButton}    45s
    sleep    2s
    wait until element is enabled    ${Close_Button}  45s
    Click Element    ${Close_Button}
    sleep    2s

Send Doc For Signing

    wait until element is enabled    ${threedotoptionforBigfile}  45s
    click element   ${threedotoptionforBigfile}
    sleep    2s
    wait until element is enabled    ${SendForSigningButton}    45s
...
Sign first document
    ...
    Wait Until Element Is Enabled    ${Approve_Document}|${Signee_Signature_Field}    25s
    click element    ${Approve_Document}|${Signee_Signature_Field}
    log to console    click on approve button
    Sleep    3s
    wait until element is enabled    //li[@ng-click="vm.selectType('upload')"]  45s
    Click Element    //li[@ng-click="vm.selectType('upload')"]
    Sleep    2s
    wait until element is enabled    ${Browse_Button}  45s
...
Sign second document
    ...
    Wait Until Element Is Enabled    ${Next_Second_Doc}    45s
    Click Element    ${Next_Second_Doc}
    sleep    7s
    Sign first document


Sign document inside signee account
...
Approving and sign doc on mailinatore side    [Arguments]    ${random_string}
    ...
    log to console    click on approve button
    sleep  10s
    wait until element is enabled    ${Add_signature_textbox}  45s
