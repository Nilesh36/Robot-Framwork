...
Click Plus button from Portals window
    ...
    wait until element is enabled    ${PlusbuttonPortals}  45s   #Wait for Plus button to load
...
Enter Name
    ...
    [Return]    ${Name}
    ${Name}    randomword
    wait until element is enabled    ${PortalNametextbox}  45s     #Wait for Portal Name field to load
...
Click Create button
    wait until element is enabled    ${Createbutton}  45s    #Wait for Create button to enable
