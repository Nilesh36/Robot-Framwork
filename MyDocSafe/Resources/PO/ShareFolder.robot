...
Click Share button
    ...
    sleep    2s
    wait until element is enabled    ${ShareButton}  45s
...
Click Add New member Button
    ...
    wait until element is enabled    ${AddNewMemberButton}    45s
    sleep    2s
...
Enter firstname in the Search for a person
    ...
    wait until element is enabled    ${SearchForaPersonTextField}  45s
...
Click Save button (Invite New member)
    ...
    wait until element is enabled    ${Savebutton}  45s    #Wait for save button to enable
    click element    ${Savebutton}    #Click Save button

Enter Last Name (Invite New Member)

    ${Lastname}    randomword
    wait until element is enabled    ${LastNameInviteNewMember}  45s
    input text    ${LastNameInviteNewMember}    ${Lastname}

Enter Email (Invite New Member)

    ${Email}    Get Email
    wait until element is enabled    ${EmailInviteNewMember}  45s
    input text    ${EmailInviteNewMember}    ${Email}

Choose Permission    [Arguments]    ${Access}    ${index}

    wait until element is enabled    ${PermissionDropDown}  45s
    click element    ${PermissionDropDown}
    sleep    2s
    wait until element is enabled    (//div[contains(text(), '${Access}')]//parent::md-option)[${index}]    45s
    click element    (//div[contains(text(), '${Access}')]//parent::md-option)[${index}]

Click Invite new member button

    wait until element is enabled    ${InviteNewMemberButton}    45s
...
Invite New member    [Arguments]    ${NewMember}
    ...
    sleep    2s
    wait until element is enabled    //span[text() = '${NewMember}']    45s
    click element    //span[text() = '${NewMember}']

Verify the folder is shared

    wait until element is enabled    ${PendingInvitationText}    45s
    sleep    2s
...
Login to 2nd account and accept invite    [Arguments]    ${FolderInviteUsername}
    ...
    sleep    1s
    Click Signin Button
    wait until element is enabled    ${AcceptInvite01}    45s
    sleep    1s
...
Verify the options available    [Arguments]    ${Option}
    ...
    wait until element is enabled    //span[text() = '${Option}']//parent::span//parent::button  45s
    sleep    2s
...
Verify the folder creation option not available
    ...
    wait until element is not visible    //h1/button/md-icon  45s
    sleep    2s
